...
*   ------------
*   Changes made:
*   1) provide a queueing mechanism to help "throttle" the
*       XHR connections.
*   2) cleanup code to attempt to physically abort any connections
*       in process upon windon unload.
*/
...
Ext.lib.Ajax = function() {     
    ...
    // Array used as a FIFO queue to store pending requests
    var _queue = [];

    // Object used to store data about requests that are in process
    var _requestsInProgress = { count: 0 };

    // Stores the number of active requests at any given point in time
    ...
    // private - modified for AJAX queueing
    function releaseObject(o) {
        // DJG - addition
        _removeRequestProcessed(o.tId);
        // DJG - end addition
        o.conn = null;
        o = null;
        _activeRequests--;
    ...
    function _processQueue() {
        var oPending = _queue[0];
        if (oPending && (_activeRequests < _concurrentRequests)) {
            oPending = _queue.shift();
            _asyncRequest(oPending);
            // cleanup?
            oPending = null;
        }
    }
    
    // private - new function for queueing
    // DJG - forum code has been overriden
    function _asyncRequest(oPending) {
    
        var method = oPending.method;
        var uri = oPending.uri;
        var callback = oPending.callback;
        var postData = oPending.postData;

        // DJG - get new connection object
        var o = getConnectionObject();
        if (!o) {
            return null;
        }
        ...
        // DJG
        _addRequestInProcess(o.tId, o.conn);
    ...
    // DJG - new method
    function _addRequestInProcess(tId, conn) {
        if (typeof(tId) != "number") {
            throw new Error(100, "Null argument in function _addRequestInProcess");
        }
        _requestsInProgress[tId] = conn;
        _requestsInProgress.count++;
    }

    // DJG - new method
    function _removeRequestProcessed(tId) {
        if (_requestsInProgress[tId]) {
            _requestsInProgress[tId] = null;   // this is now the xhr request
            delete _requestsInProgress[tId];
            _requestsInProgress.count--;
        }
    }
    ...
    // DJG - new private function
    // Called from window.onunload
    function _abortConnectionOnWindowClose(tId, conn) {
        try {
            var poll = pub.poll;
            if (conn.readyState != 4 && conn.readyState != 0) {
                // connection is in progress
                // this is NOT a timeout and we do not have a callback
                conn.abort();
                clearInterval(poll[tId]);
                delete poll[tId];
                conn = null;
                ...
                // we don't care about cleaning up the queue here
                return false;
            }
        }
        catch (e) { }
...
//          },
            ...
            abort : function(o, callback, isTimeout) {
                ...
                if (me.isCallInProgress(o)) {
                    ...
                    if (isTimeout) {
                        me.timeout[tId] = null;
                        // DJG - addition
                        _removeRequestProcessed(tId)
                        // DJG - end addition
                    }
                    handleTransactionResponse(o, callback, (isAbort = true), isTimeout);
                }
                else {
                    /* DJG: look at this new code? */
            ...
            // DJG - new function to cleanup any open connections
            //      as well as requests that are queued
            cleanup : function() {
                try {
                    // cleanup open XHR requests
                    if (_requestsInProgress.count > 0) {
                        for (var prop in _requestsInProgress) {
                            if (prop != "count") {
                                _abortConnectionOnWindowClose(prop, _requestsInProgress[prop]);
                                _requestsInProgress[prop] = null;
                                delete _requestsInProgress[prop];
                            }
                        }
                    }
                    _requestsInProgress = null;
