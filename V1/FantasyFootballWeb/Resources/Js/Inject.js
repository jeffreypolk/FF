        ...
        var external = {
            ...
            yahoo: {
                ...
                getPoints: function () {
                    ...
                    $('.ysf-player-name').each(
                        function () {
                            ...
                            //debugger;
                            var points = parseFloat($(this).parent().parent().parent().parent().find('.Fw-b').html());
                ...
                getPlayers: function () {
                    ...
                    var players = [];
                    var table = $('.Table.Ta-start.Fz-xs.Table-mid.Table-px-sm.Table-interactive');
                    ...
                    table.find('tbody tr').each(
                        function () {
                            ...
                            var team = tr.find('.ysf-player-name span').text().split(' - ')[0].toString().toUpperCase();
                            var pos = tr.find('.ysf-player-name span').text().split(' - ')[1];
                            var points = $(tr.find('td')[7]).find('span').text();
                            if (!points){ 
                                points = 0;
                            }
                            sql.push('INSERT INTO FF.Player ([Name], [NFLTeam], [Position], [Age], [Experience], [ADP], [DepthChart], [Year], [ActualPoints], [ProjectedPoints], [Odds])');
                            sql.push('\n');
                            sql.push('VALUES (\'', name.replaceAll('\'', '\'\''), '\', \'', team, '\', \'', pos, '\', 0, 0, 0, 1, year(getdate()), 0, ', points, ', 0)');
                            sql.push('\n');
                        }
                    );
                    //console.log(sql);
                    ...
                    rows.each(
                        function () {
                            ...
                            if (tr.hasClass('division') === false) {
                                ...
                                var finish = $(tds[pos.finish]).text().toString().replaceAll('*', '').replaceAll('.', '');
                                ...
                                sql.push('WHERE [Year] = ', year, ' AND [Name] = \'', name.toString().replaceAll('\'', '\'\''), '\'');
                                sql.push('\n');
                            }
